...
if __name__ == "__main__":
    ...
    for i in range(num_sats):
        ...
        for j in range(simLength):
            ...
            if trans_earth == False:
                if satAER[c][1,j] < 0:
                    satAER[c][:,j:j+1] = np.array([[np.nan],[np.nan],[np.nan]])
                    
        if np.isnan(satAER[c]).all():
            print('Satellite {s} is not observiable'.format(s=c))
        
    
    # Add small deviations for measurements
    # Using caluclated max measurement deviations for LT:
    ...
    for i in range(num_sats):
        c = chr(i + 97)
        for j in range(simLength):
            satECIMes[c][:,j:j+1] = Transformations.AERtoECI(satAERMes[c][:,j], stepLength, j, sensECEF, sensLLA[0], sensLLA[1])
    ...
    for i in range(num_sats):
        ...
        for j in range(simLength):
            ...
            satState[c], covState[c] = Filters.EKF_ECI(satState[c], covState[c], satECIMes[c][:,j], stateTransMatrix, measureMatrix, covECI, procNoise)
